...
#+begin_src emacs-lisp
  (use-package corfu
    ...
    (setq corfu-left-margin-width 0.0)
    (setq corfu-right-margin-width 0.0)
    ...
    (add-hook 'exwm-mode-hook
              (lambda ()
                (add-hook 'pre-command-hook #'corfu-quit nil t)))
    (advice-add #'corfu--make-frame :after
                (lambda (&rest _)
                  (when (frame-live-p corfu--frame)
                    (set-frame-parameter corfu--frame 'parent-frame (selected-frame))
